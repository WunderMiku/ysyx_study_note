...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));  // RV64I中 结果需要进行符号扩展
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> (imm & 0x3f)); // RV32I 中 imm[5]必须为0
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << (src2 & 0x1f)); // RV64I 中应为src2低6位为位移位数
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm & 0x3f)); // RV32I 中 imm[5]必须为0
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if (src1 >= src2) s->dnpc = s->pc + imm);
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & 0x3f)); // RV32I 中 imm[5]必须为0
  ...
  // 这里使用了int64_t来进行乘法运算，避免了乘法溢出的问题（但如果是RV64I的话，可能需要更改这里与右移位数）
  // 正确的符号扩展必须显式地先转换为 int32_t 再转换为 int64_t (TODO: 需要查证)
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (sword_t)src1 >> (src2 & 0x1f)); // RV64I 中应为src2低6位为位移位数
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> (src2 & 0x1f)); // RV64I 中应为src2低6位为位移位数
