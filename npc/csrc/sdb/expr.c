  ...
  *success = true;
  ...
  if (p == q) { // 仅存在一个token 若为 NUM/HEX_NUM 类型返回其数值，如果是寄存器类型 返回其寄存器值 若都不是则不合法
    ...
    if((tokens[p].type == NUM) || (tokens[p].type == HEX_NUM)) {
      ...
      //printf("Return NUM: %s = %u", tokens[p].str, val);
      return val;
    }
    ...
    if(tokens[p].type == TK_REGS) {
      ...
      if(*success) {
      //printf("Return Reg: $%s = 0x%08x", tokens[p].str, val);
      return val;
      } else {
    ...
    // 不合法
    printf("Single token is not a number or reg: type=%d\n", tokens[p].type);
    *success = false;
    return 0;
  ...
  if (!check_parentheses_valid(p, q)) { // 括号不合法
    // printf("Parentheses not valid");
    *success = false;
    return 0;
  }

  if (check_parentheses(p, q)) { // 该表达式被一对括号包括，需要去除该对括号
    // printf("Parentheses enclosed, recursing into (%d, %d)", p+1, q-1);
  ...
  // printf("Operator position: %d, type: %d", op, op_type);
  ...
  switch (op_type) {
    case '+': 
      result = val1 + val2;
      // printf("Computing: %u + %u = %u", val1, val2, result);
      break;
    ...
    case DEREF:
      //检查地址是否越界
      if (val2 < MEM_BASE || val2 > MEM_BASE + MEM_SIZE - 1) { 
        printf("Address %d is out of bounds. Valid range: [%d, %d]\n"\
          ,val2, MEM_BASE, MEM_BASE + MEM_SIZE - 1);
        return 0;
      }
      result = (uint32_t) paddr_read(val2, 4);
      break;
      
    case TK_EQ:
      result = (val1 == val2);
       // printf("Comparing: %u == %u = %s", val1, val2, result ? "true" : "false");
      break;
    ...
    case TK_AND:
      result = (val1 && val2);
      // printf("Logic AND: %u && %u = %s", val1, val2, result ? "true" : "false");
      break;
  ...
  // printf("Returning result: %u", result);
  return result; 
}
