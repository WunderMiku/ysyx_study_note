...
void funget_detect(uint32_t before_pc, uint32_t pc, uint32_t inst) {
	...
	if(fun_get.is_call) {
		...
		while(i-- > 0) printf(" ");
		printf(COLOR_GREEN " (Call) " COLOR_NONE COLOR_YELLOW "%s" COLOR_NONE "(0x%x) -> " COLOR_GREEN "%s"\
	...
	if(fun_get.is_ret) {
		...
		int i = fun_get.call_level - 1;
		while(i-- > 0) printf(" ");
		printf(COLOR_YELLOW " (Ret) " COLOR_NONE COLOR_GREEN "%s" COLOR_NONE "(0x%x) <- " COLOR_YELLOW "%s"\
	...
	if(fun_get.is_move && !fun_get.is_ret) {
		int i = fun_get.call_level - 1;
		while(i-- > 0) printf(" ");
		printf(COLOR_CYAN " (Jmp) " COLOR_NONE COLOR_BLACK "%s -> %s \n"\
...
{
	printf(COLOR_GREEN "Getting function info from "  COLOR_YELLOW "%s\n" COLOR_NONE, file);
	if(file == NULL) {
    printf("NO files\n");
		exit(1);
	}
	...
	// 解析符号表信息
	parse_symbol_table((uint8_t *) map_addr);
}


uint8_t *parse_string_table(const uint8_t *addr) {
	...
	// 获取节头表基地址
	uint8_t *shdr_base = (uint8_t *)(addr + shoff);
	...
	uint8_t *shstr_base = (uint8_t *)(addr + shstrhdr->sh_offset);
...
void parse_symbol_table(const uint8_t *addr) {
	...
	uint8_t *shstr_base = parse_string_table(addr);
	uint8_t *sym_base = NULL;
	uint8_t *strtab_base = NULL;

	int sym_num = 0;

	for(int i = 0; i < shnum; i++) {
		const Elf32_Shdr *shdr = (const Elf32_Shdr *)(shdr_base + i * shentsize);
		const char *section_name = (char *)shstr_base + shdr->sh_name;
		// printf("Section header %d at: %p\n", i, shdr);
		if(strcmp(section_name, ".symtab") == 0) {
			sym_base = (uint8_t *)(addr + shdr->sh_offset);
			sym_num = shdr->sh_size / sizeof(Elf32_Sym);
		}
		if(strcmp(section_name, ".strtab") == 0) {
			strtab_base = (uint8_t *)(addr + shdr->sh_offset);
		}
	}
...
// 输出elf节头表
void parse_section_header_table(const uint8_t *addr) {
	const Elf32_Ehdr *ehdr = (const Elf32_Ehdr *)addr;
